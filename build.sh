...
CASDOOR_REF="${CASDOOR_REF:-master}"

usage() {
  cat <<USAGE
Usage: bash build.sh --arch amd64|arm64|all [--casdoor-ref <git-ref>]

Build Casdoor offline .run installer packages.

Options:
  --arch <arch>             Target architecture: amd64, arm64, or all.
  --casdoor-ref <git-ref>   Upstream casdoor/casdoor branch, tag, or commit. Default: ${CASDOOR_REF}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --arch) ARCH="${2:-}"; shift 2 ;;
    --casdoor-ref) CASDOOR_REF="${2:-}"; shift 2 ;;
    -h|--help) usage; exit 0 ;;
...
[[ -n "${CASDOOR_REF}" ]] || die "--casdoor-ref cannot be empty"
...
build_one() {
  ...
  echo ">>> building ${PACKAGE_NAME} ${VERSION} for ${arch}, upstream ref=${CASDOOR_REF}"
  rm -rf "${build_dir}"
...
PY
    ...
    if [[ -n "${dockerfile}" ]]; then
      ...
      echo ">>> docker buildx build --load --platform ${platform} -t ${tag} -f ${dockerfile} --build-arg CASDOOR_REF=${CASDOOR_REF}"
      docker buildx build --load --platform "${platform}" \
        --build-arg "CASDOOR_REF=${CASDOOR_REF}" \
        -t "${tag}" \
